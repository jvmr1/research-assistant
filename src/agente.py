...
pastas, modelos padrão, consultas padrão, leitura de
obsidian/ANOTACOES.md e funções simples para criar/atualizar notas Markdown de artigos. O fluxo pesado
fica em src/motor.py, src/revisao.py e src/apresentacao.py para evitar um
arquivo único enorme.
...
ARTIGOS = REFERENCIAS / "fichamentos"
PDFS = REFERENCIAS / "pdfs"
# Pasta legada mantida só por compatibilidade. Os trabalhos-base agora ficam
# fisicamente em obsidian/referencias/pdfs e são identificados pela lista em ANOTACOES.md.
EXEMPLOS = PDFS
...
INSTRUCOES_PADRAO = """# Anotações

Este é o arquivo principal de conversa com a IA. Escreva aqui o rumo da
...
Coloque todos os PDFs em `obsidian/referencias/pdfs/`. Quando algum PDF for trabalho-base
ou exemplo fornecido pelo pesquisador, liste o nome do arquivo aqui. O agente
deve tratá-lo como semente prioritária, fazer leitura integral e usar suas
ideias para orientar novas buscas. Esses trabalhos também podem orientar a
estrutura da redação, mas não são copiados nem tratados como citações
automáticas.
...
def garantir_instrucoes_iniciais():
    """Garante o arquivo único de anotações.

    Em clones antigos, migra o conteúdo de obsidian/INSTRUCOES.md sem apagar nada;
    a limpeza dos arquivos legados fica a cargo da rotina de apresentação.
    """
    if INSTRUCOES.exists():
        return
    INSTRUCOES.parent.mkdir(parents=True, exist_ok=True)
    if INSTRUCOES_LEGADO.exists():
        legado = INSTRUCOES_LEGADO.read_text(encoding="utf-8-sig")
        INSTRUCOES.write_text(
            "# Anotações\n\n"
            "Conteúdo migrado do antigo `INSTRUCOES.md`.\n\n"
            + legado,
            encoding="utf-8",
        )
    else:
